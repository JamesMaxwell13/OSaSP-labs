...
int main(int argc, char *argv[])
{
    if (argc != 5)
    {
        ...
        fprintf(stdout, "USAGE: ./sort_index <memory size> <blocks amount> <threads amount> <file name>\n");
        return EXIT_FAILURE;
    }
    ...
    if (size % pagesize)
    {
        fprintf(stderr, "Memory size must be a multiple of the page size!\n");
        return EXIT_FAILURE;
    }

    if ((amount_of_blocks & (amount_of_blocks - 1)))
    {
        fprintf(stderr, "Amount of blocks must be a power of 2\n");
        return EXIT_FAILURE;
    }

    if (amount_of_blocks <= amount_of_threads)
    {
        fprintf(stderr, "Amount of blocks must be greater than amount of threads\n");
        return EXIT_FAILURE;
    }
    ...
    {
        fprintf(stderr, "Amount of threads must be between %d and %d\n", core, N);
        return -1;
    }
    ...
    printf("\n");
    printf("%s was sorted and saved to %s\n\n", filename, SORTED_FILENAME);
